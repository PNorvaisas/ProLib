...
fragcompEM<-ggplot(subset(allfrags,!is.na(Lg_Ki_M) & Volume_type %in% c('Empty_VdW')),aes(x=P,y=-Lg_Ki_M,color=Crop))+geom_point()+stat_smooth(method = "lm")
fragcompEM+facet_grid(.~Volume_type,scales='free_x')
