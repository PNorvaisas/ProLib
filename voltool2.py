...
def multiprocessor(func,args,inps):
...
def functions_M(func,odata,cores):
	...
	#print args
	#sys.exit(1)
	results=multiprocessor(func,args,inps)
...
def mcvolhandler((links_frag,settings,q)):
	#Multi-threaded execution McVol handler
	outputs=NestedDict()
	for k in links_frag.keys():
		for f in links_frag[k]:
			if f!='' and os.path.isfile(f):
				fpath, fname, ftype=filename(f)
				if '_' in fname:
					part=fname.split('_')[-1]
				...
				if not fail:
					...
					print '{}, {} has failed!'.format(k,part)
			else:
	...
	for k in links.keys():
		...
		for f in files:
			...
			if '_' in fname:
				...
				part='vol'
			inps.append('{}_{}'.format(k,part))

	blist=slice_list(links.keys(),cores)
	for slice in blist:
		links_frag={lk : links[lk] for lk in slice}
		args.append((links_frag,settings,q))

	#print args
	results=multiprocessor(mcvolhandler, args,inps)
...
def prepare_M(ilist,pdbdata,pqr,cores):
	#Generates list of actual files from the input string
	if len(pdbdata.keys())==0:
		pdbdata=NestedDict()
	links={}
	...
	#print args
	#sys.exit(1)
	results=multiprocessor(prepare_handler,args,ilist)
	...
	for res in results:
		...
		prc=itr*100/len(results)
		print "{0:3d}%".format(int(prc))
		links=mapmerge(links,res[0])
		pdbdata=mapmerge(pdbdata,res[1])
...
def splitpdb_silent(ifile, odata, ligname=''):
	#PDB parsing engine that let's the user select PDB chain, ligand or ligands and saves their choices.
	info=NestedDict()
	ipath, iname, itype=filename(ifile)
	u=MD.Universe(ifile)
